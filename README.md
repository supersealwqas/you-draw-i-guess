...
### v7.0 - 流式输出 & Markdown 渲染 & 推理透视 (当前版本)
**更新时间**：2026-04-11
- **流式输出 (Streaming)**：后端改造为 SSE 管道架构，AI 回答文字**逐字实时输出**，告别等待黑屏。同时兼容千问云端 SSE 格式与 Ollama 本地 NDJSON 格式。
- **Markdown 富文本渲染**：集成 `marked.js` + `DOMPurify`，AI 输出的标题、列表、表格、代码块等 Markdown 语法**实时编译为精美排版**。
- **代码语法高亮**：集成 `highlight.js` (Atom One Dark 主题)，代码块自动识别语言并着色，极客感满满。
- **推理过程可视化**：
    - 支持检测千问 `reasoning_content`、Ollama 原生 `thinking` 字段及文本内嵌 `<think>` 标签。
    - 思考过程实时展示在可折叠面板中，思考阶段自动展开，正文输出后自动折叠。
    - 面板固定高度 300px 并自动滚底，保证最新思考内容始终可见。
- **打字机光标**：流式输出时，文本末尾附带闪烁光标，输出完毕后自动消失。
- **交互防护**：AI 思考/输出过程中，所有操作按钮自动禁用防止误触，完成后恢复。

### v6.2 - 功能完善与交互升级
**更新时间**：2026-04-11
- **画作保存功能**：在画板工具栏新增"💾 保存"按钮，支持一键将当前画作下载为本地 `.png` 高清图片。
- **自定义提示词重构**：采用"渐进式引导"交互，"一键生成描述" + "我想知道更多..."双按钮分支设计。
...
## 📦 技术栈
- **Frontend**: Vanilla HTML / CSS / JavaScript + marked.js + DOMPurify + highlight.js
- **Backend**: Node.js / Express / Axios / dotenv / SSE Streaming
- **AI Support**: Ollama (gemma3, qwen3.5) + 阿里云 Qwen API (qwen3.5-flash, qwen3.5-plus, qwen3.6-plus)
